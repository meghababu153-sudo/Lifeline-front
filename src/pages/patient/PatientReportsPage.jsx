...
function ReportViewerModal({ report, onClose }) {
  ...
  // fileRef is a blob URL set during upload (present for newly uploaded files)
      ...
      >
        ...
        <div className="flex items-center justify-between bg-slate-900 text-white px-6 py-4 shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {report.reportType} · {report.uploaderName} ·{" "}
        ...
        <div className="flex-1 overflow-auto bg-slate-800 p-6">
          {fileUrl && isPDF ? (
            ...
            /* Fallback: show document information when no blob URL is available
               (for pre-loaded demo data that doesn't have real file blobs) */
...
function PatientReportsPage() {
  ...
  const handleViewReport = (report) => {
    addAuditLog({
      ...
      details: `Viewed original report ${report.fileName}`,
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="flex flex-wrap gap-3 mt-1.5 text-sm text-slate-500">
                          ...
                          {r.extracted?.hospitals?.[0] && (
                            <span>· {r.extracted.hospitals[0]}</span>
                          )}
                        </div>
                      </div>
